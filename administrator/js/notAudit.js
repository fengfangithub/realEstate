    ...
    function allreleaseInformationBack(data){
        ...
        if(data.state == 0){
            ...
            for(var i =(page_num - 1)*rows; i < page_num*rows && i < resultData.length; i++){
                ...
                var audit_a = $(audit_td[i]).find("a");
                audit_a.click(function () {
                    var house_id = id;
                    return function () {
                        dataLoad("http://www.xhban.com:8080/EM/admin/permithouse",{house_id: house_id},auditBack);
                    }
                }());
            }
            function auditBack(data) {
                console.log(data)
            ...
            for (var j = 1; j < a.length-1; j++){
                $(a[j]).click(function () {
                    var n = j;
                    return function () {
                        var url = "notAudit.html?page_num="+n+"&rows="+rows+"&user_id="+user_id;
            ...
            $(a[0]).click(function () {
                if(page_num-1>=1){
                    var url = "notAudit.html?page_num="+(page_num-1)+"&rows="+rows+"&user_id="+user_id;
                    window.location.href = url;
                }
            });
